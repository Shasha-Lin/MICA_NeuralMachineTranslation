...
parser.add_argument('--use_cuda', action='store_true', help='IF USE CUDA (Default == True)')
...
def prepare_data(lang1_name, lang2_name, do_filter=True, MIN_LENGTH=opt.MIN_LENGTH, MAX_LENGTH=opt.MAX_LENGTH, reverse=False, char_output=False):

    # Get the source and target language class objects and the pairs (x_t, y_t)
    input_lang, output_lang, pairs = read_langs(lang1_name, lang2_name, term=opt.model_type, reverse=reverse, char_output=char_output)
...
def variable_from_sentence(lang, sentence, char=False):
