...
do
    ...
    sbatch run_all_jobs.sh "bpe2bpe" "${learning_rate[$i]}" "$teacher_forcing_ratio" "${embed_dim[$i]}" "${hidden_size[$i]}" "${k_beam_search[$i]}" "$n_layers" "$code_dir"
