...
def get_max_iteration(): 
    ...
    files = os.listdir("{}/{}/".format(opt_rerun.out_dir, opt_rerun.experiment_name))
    iterations = [get_number(x) for x in files]
    
    return np.max(iterations)

if opt_rerun.continue_from is not None: 
    epoch = opt_rerun.continue_from 
else: 
...
enc_state = torch.load("{}/{}/saved_encoder_{}.pth".format(opt_rerun.out_dir, opt_rerun.experiment_name, epoch))
encoder.load_state_dict(enc_state)

print("Loading/Updating decoder state dict at epoch {}".format(epoch))
dec_state = torch.load("{}/{}/saved_decoder_{}.pth".format(opt_rerun.out_dir, opt_rerun.experiment_name, epoch))
decoder.load_state_dict(dec_state)


print("Loading states of optimizers")
try: 
    enc_optimizer_state = torch.load("{}/{}/encoder_optimizer_{}.pth".format(opt_rerun.out_dir, opt_rerun.experiment_name, epoch))
    encoder_optimizer.load_state_dict(enc_optimizer_state)
    dec_optimizer_state = torch.load("{}/{}/decoder_optimizer_{}.pth".format(opt_rerun.out_dir, opt_rerun.experiment_name, epoch))
    decoder_optimizer.load_state_dict(dec_optimizer_state)
except FileNotFoundError: 
	# Patch because some models have no optimizers states saved
    print("Optimizers states not found. Model running with no optimizer update")
